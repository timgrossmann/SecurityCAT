...
extra_headers = {
    "Access-Control-Allow-Origin": SECRAT_URL,
...
scan_definition = api.model(
    "ScanDefinition",
    {
        "testProperties": fields.Nested(
            api.model(
                "TestProperties",
                {
        ...
        "requirements": fields.List(
            fields.String,
            required=True,
            description="List of requirements short names to test",
        ),
    },
)
...
class StartTest(Resource):
    @api.doc(
        responses={
            202: "Scan was accepted, gives back the location header for fetching the result"
        }
    )
