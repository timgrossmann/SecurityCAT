...
for req in requirements:  # requirements passed from over gateway from securityRAT
    ...
    if req == "ASVS_3.0.1_10.10":
        if "Public-Key-Pins" in response.headers:
            ...
            output_res["result"]["message"] = (
                "Detected HPKP Header with the value: \n```\n"
        ...
        if "Strict-Transport-Security" in response.headers:
            output_res["result"]["status"] = "PASSED"
            output_res["result"]["message"] = (
                "Detected HSTS Header with the value: \n```\n"
        ...
        if (
            ...
            and "preload" in response.headers["Strict-Transport-Security"]
        ):
            output_res["result"]["status"] = "PASSED"
            output_res["result"]["message"] = (
                "Preloading active: : \n```\n"
